...
/**
 * PHP program used to benchmark AngstromDB server amongst other databases.
 *
...
for ($i = 0; $i < $nbrLoops; $i++) {
	foreach ($dataList as $key => $data) {
		$sock = fsockopen(ANGSTROM_HOST, ANGSTROM_PORT);
		/*$result =*/ angstromGet($sock, "$key-$i", $data);
		fclose($sock);
		//if ($result != $data)
		//	print("GET error '$result' / '$data'\n");
	}
}
$timer->stop();
...
for ($i = 0; $i < $nbrLoops; $i++) {
	foreach ($dataList as $key => $data) {
		$sock = fsockopen(FINEDB_HOST, FINEDB_PORT);
		/*$result =*/ commandGet($sock, false, "$key-$i", $data);
		fclose($sock);
		//if ($result != $data)
		//	print("GET error '$result' / '$data'\n");
	}
 ...
 */
function angstromPut($sock, $key, $data) {
...
function commandGet($sock, $compress, $key, $check=null) {
	...
	$length = substr($response, 1, 4);
	$length = unpack('Nint', $length);
	$length = $length['int'];
	if ($length != (mb_strlen($response, 'ascii') - 5)) {
		print("GET error (wrong size)\n");
		return;
	}
